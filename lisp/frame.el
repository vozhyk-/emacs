...
recently selected windows nor the buffer list."
  ...
  (when (memq (window-system frame) '(x w32 ns gtk3wl))
    (x-focus-frame frame))
...
`iconify-or-deiconify-frame' from an X frame."
  ...
  (let ((type (framep (selected-frame))))
    (cond
     ((memq type '(x ns w32 gtk3wl)) (iconify-or-deiconify-frame))
     ((eq type t)
...
(declare-function gtk3wl-frame-geometry "gtk3wlfns.c" (&optional frame))
...
`internal-border-width' is the width of the internal border of
  ...
  (let* ((frame (window-normalize-frame frame))
    ...
    (cond
     ...
     ((eq frame-type 'gtk3wl)
      (gtk3wl-frame-geometry frame))
     (t
      (list
       '(outer-position 0 . 0)
...
(declare-function gtk3wl-frame-edges "gtk3wlfns.c" (&optional frame type))
...
(declare-function gtk3wl-mouse-absolute-pixel-position "gtk3wlfns.c")
...
position (0, 0) of the selected frame's terminal."
  (let ((frame-type (framep-on-display)))
    (cond
     ...
     ((eq frame-type 'gtk3wl)
      (gtk3wl-mouse-absolute-pixel-position))
     (t
      (cons 0 0)))))

(declare-function gtk3wl-set-mouse-absolute-pixel-position "gtk3wlfns.c" (x y))
...
position (0, 0) of the selected frame's terminal."
  (let ((frame-type (framep-on-display)))
    (cond
     ((eq frame-type 'gtk3wl)
      (gtk3wl-set-mouse-absolute-pixel-position x y))
     ((eq frame-type 'ns)
...
(declare-function gtk3wl-frame-list-z-order "gtk3wlfns.c" (&optional display))
...
Return nil if DISPLAY contains no Emacs frame."
  (let ((frame-type (framep-on-display display)))
    (cond
     ...
     ((eq frame-type 'gtk3wl)
      (gtk3wl-frame-list-z-order display)))))
...
(declare-function gtk3wl-frame-restack "gtk3wlfns.c" (frame1 frame2 &optional above))
  ...
  (if (and (frame-live-p frame1)
      ...
      (let ((frame-type (framep-on-display frame1)))
        (cond
         ...
         ((eq frame-type 'gtk3wl)
          (gtk3wl-frame-restack frame1 frame2 above))))
...
DISPLAY can be a display name, a frame, or nil (meaning the selected
frame's display)."
  (not (null (memq (framep-on-display display) '(x w32 ns gtk3wl)))))
...
frame's display)."
  (let ((frame-type (framep-on-display display)))
    (cond
     ...
     ((memq frame-type '(x w32 ns gtk3wl))
      t)
     (t
      nil))))
...
monitor, use `display-monitor-attributes-list'."
  (and (memq (framep-on-display display) '(x w32 ns gtk3wl))
...
(declare-function ns-display-monitor-attributes-list "nsfns.m"
		  (&optional terminal))
(declare-function gtk3wl-display-monitor-attributes-list "gtk3wlfns.c"
		  (&optional terminal))
...
monitors."
  (let ((frame-type (framep-on-display display)))
    (cond
     ...
     ((eq frame-type 'gtk3wl)
      (gtk3wl-display-monitor-attributes-list display))
     (t
...
terminals, cursor blinking is controlled by the terminal."
  :init-value (not (or noninteractive
		       ...
		       (not (memq window-system '(x w32 ns gtk3wl)))))
